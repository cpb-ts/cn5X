...
from PyQt5.QtCore import QObject, QSettings, Qt, pyqtSignal, pyqtSlot
from PyQt5.QtGui import QPalette, QStandardItem, QStandardItemModel, QValidator
...
class dlgG28_30_1(QObject):
    ...
    def __init__(self, Gpos, grbl: grblCom, decoder, axisNumber: int, axisNames: list):
        ...
        if self.__Gpos[-2:] == ".1":
            self.__dlg.setWindowTitle(
                self.tr("Define {} absolute position?").format(self.__Gpos)
            )
            self.di.lblPosition.setText(self.tr("Current machine position (MPos)"))
            self.di.lblMessage.setText(
                self.tr(
                    "Save the current machine position (MPos) in the {} Grbl's location?"
                ).format(self.__Gpos)
            )
            self.enableDisableCheckBoxes(False)
            # Affichage confirmation ou non...
            self.__dontConfirm = self.__settings.value(
                "dontConfirm{}".format(self.__Gpos), False, type=bool
            )
            self.di.chkDontShow.setChecked(self.__dontConfirm)
            self.di.chkDontShow.setVisible(True)
        else:
            self.__dlg.setWindowTitle(
                self.tr("Go to {} stored location?").format(self.__Gpos)
            )
            self.di.lblPosition.setText(
                self.tr("Current {} defined position").format(self.__Gpos)
            )
            text = self.tr(
                "Make a rapid move from current location to the position defined by the last {}?\n".format(
                    self.__Gpos
                )
            )
            text += self.tr(
                "If no positions are stored with {} then all axes will go to the machine origin.".format(
                    self.__Gpos
                )
            )
        ...
        self.di.imageDeco.setIcon(
            QtGui.QIcon(":/cn5X/images/question{}.svg".format(self.__Gpos))
        )
        ...
        if self.__Gpos[-2:] == ".1":
            ...
            self.di.lblPosX.setText(
                "{:+0.3f}".format(self.__decode.getMpos(self.__axisNames[0]))
            )
            self.di.lblPosY.setText(
                "{:+0.3f}".format(self.__decode.getMpos(self.__axisNames[1]))
            )
            self.di.lblPosZ.setText(
                "{:+0.3f}".format(self.__decode.getMpos(self.__axisNames[2]))
            )
            if self.__nbAxis > 3:
                self.di.lblPosA.setText(
                    "{:+0.3f}".format(self.__decode.getMpos(self.__axisNames[3]))
                )
            else:
                self.di.lblPosA.setText("-")
            if self.__nbAxis > 4:
                self.di.lblPosB.setText(
                    "{:+0.3f}".format(self.__decode.getMpos(self.__axisNames[4]))
                )
            else:
                self.di.lblPosB.setText("-")
            if self.__nbAxis > 5:
                self.di.lblPosC.setText(
                    "{:+0.3f}".format(self.__decode.getMpos(self.__axisNames[5]))
                )
            else:
                self.di.lblPosC.setText("-")
        else:
            if self.__Gpos[:3] == "G28":
                ...
                self.di.lblPosX.setText(
                    "{:+0.3f}".format(self.__decode.getG28(self.__axisNames[0]))
                )
                self.di.lblPosY.setText(
                    "{:+0.3f}".format(self.__decode.getG28(self.__axisNames[1]))
                )
                self.di.lblPosZ.setText(
                    "{:+0.3f}".format(self.__decode.getG28(self.__axisNames[2]))
                )
                if self.__nbAxis > 3:
                    self.di.lblPosA.setText(
                        "{:+0.3f}".format(self.__decode.getG28(self.__axisNames[3]))
                    )
                else:
                    self.di.lblPosA.setText("-")
                if self.__nbAxis > 4:
                    self.di.lblPosB.setText(
                        "{:+0.3f}".format(self.__decode.getG28(self.__axisNames[4]))
                    )
                else:
                    self.di.lblPosB.setText("-")
                if self.__nbAxis > 5:
                    self.di.lblPosC.setText(
                        "{:+0.3f}".format(self.__decode.getG28(self.__axisNames[5]))
                    )
                else:
                    self.di.lblPosC.setText("-")
            else:
                ...
                self.di.lblPosX.setText(
                    "{:+0.3f}".format(self.__decode.getG30(self.__axisNames[0]))
                )
                self.di.lblPosY.setText(
                    "{:+0.3f}".format(self.__decode.getG30(self.__axisNames[1]))
                )
                self.di.lblPosZ.setText(
                    "{:+0.3f}".format(self.__decode.getG30(self.__axisNames[2]))
                )
                if self.__nbAxis > 3:
                    self.di.lblPosA.setText(
                        "{:+0.3f}".format(self.__decode.getG30(self.__axisNames[3]))
                    )
                else:
                    self.di.lblPosA.setText("-")
                if self.__nbAxis > 4:
                    self.di.lblPosB.setText(
                        "{:+0.3f}".format(self.__decode.getG30(self.__axisNames[4]))
                    )
                else:
                    self.di.lblPosB.setText("-")
                if self.__nbAxis > 5:
                    self.di.lblPosC.setText(
                        "{:+0.3f}".format(self.__decode.getG30(self.__axisNames[5]))
                    )
                else:
                    self.di.lblPosC.setText("-")
        ...
        self.di.buttonBox.button(QDialogButtonBox.Cancel).clicked.connect(
            self.__dlg.close
        )
        self.di.chkPosX.stateChanged.connect(
            lambda: self.on_chkPos_changed(self.di.chkPosX, 0)
        )
        self.di.chkPosY.stateChanged.connect(
            lambda: self.on_chkPos_changed(self.di.chkPosY, 1)
        )
        self.di.chkPosZ.stateChanged.connect(
            lambda: self.on_chkPos_changed(self.di.chkPosZ, 2)
        )
        self.di.chkPosA.stateChanged.connect(
            lambda: self.on_chkPos_changed(self.di.chkPosA, 3)
        )
        self.di.chkPosB.stateChanged.connect(
            lambda: self.on_chkPos_changed(self.di.chkPosB, 4)
        )
        self.di.chkPosC.stateChanged.connect(
            lambda: self.on_chkPos_changed(self.di.chkPosC, 5)
        )
    ...
    def showDialog(self):
        if not self.__dontConfirm:
            ...
            self.__dlg.setFixedSize(
                self.__dlg.geometry().width(), self.__dlg.geometry().height()
            )
            self.__dlg.move(
    ...
    def on_btnYes(self):
        ...
        self.__settings.setValue(
            "dontConfirm{}".format(self.__Gpos), self.di.chkDontShow.isChecked()
        )
    ...
    def enableDisableAxis(self, axis: str, enabling: bool):
        ...
        lblLblPos = self.di.frmMPos.findChild(
            QtWidgets.QLabel, "lblLblPos{}".format(axis)
        )
