    ...
    def g38(
        ...
        if P < 2 or P > 5:
            self.sig_log.emit(
                ...
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'P' must be 2, 3, 4 or 5 for respectively G38.2, G38.3, G38.4 or G38.5."
                ),
            )
            return
        if (X is not None) and ("X" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'X' is not in the axisNames list."
                ),
            )
            raise ValueError("X")
            return
        if (Y is not None) and ("Y" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'Y' is not in the axisNames list."
                ),
            )
            raise ValueError("Y")
            return
        if (Z is not None) and ("Z" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'Z' is not in the axisNames list."
                ),
            )
            raise ValueError("Z")
            return
        if (A is not None) and ("A" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'A' is not in the axisNames list."
                ),
            )
            raise ValueError("A")
            return
        if (B is not None) and ("B" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'B' is not in the axisNames list."
                ),
            )
            raise ValueError("B")
            return
        if (C is not None) and ("C" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'C' is not in the axisNames list."
                ),
            )
            raise ValueError("C")
            return
        if (U is not None) and ("U" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'U' is not in the axisNames list."
                ),
            )
            raise ValueError("U")
            return
        if (V is not None) and ("V" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'V' is not in the axisNames list."
                ),
            )
            raise ValueError("V")
            return
        if (W is not None) and ("W" not in self.__axisNames):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: 'W' is not in the axisNames list."
                ),
            )
            raise ValueError("W")
            return
        if F <= 0:
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): ArgumentError: Probe speed: F undefinied or null."
                ),
            )
            raise speedError
            return
        ...
        if (not self.__grblCom.isOpen) or (not self.__grblCom.grblInitStatus):
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): Error: Grbl is not connected or not initialized."
                ),
            )
        ...
        if self.__decode is None:
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblProbe.g38(): cn5X++ error: no active Grbl decoder, this should never happen!?!?"
                ),
            )
            raise InternalError
            return
        ...
        for v in RC[1]:
            if num > 5:
                self.sig_log.emit(
                    ...
                    self.tr(
                        "grblProbe.on_sig_probe(): Warning: Grbl probe response have more than 6 axis. Values of axis number > 6 are ommited!"
                    ),
                )
                break
            ...
            num += 1
        self.sig_log.emit(
            logSeverity.info.value, self.tr("grblProbe.g38(): Response probe received.")
        )
        ...
        if RC[0]:
            ...
            if RC[2] == SIG_ERROR:
                self.sig_log.emit(
                    logSeverity.error.value,
                    self.tr("grblProbe.g38(): Probe error: error received!"),
                )
                ...
                self.sig_log.emit(
                    logSeverity.error.value,
                    self.tr("grblProbe.g38(): Probe error: Alarm! received"),
                )
                raise probeError(probeGCode)
                return
            elif RC[2] == SIG_PROBE:
                self.sig_log.emit(
                    logSeverity.error.value,
                    self.tr("grblProbe.g38(): Error: Last probe failure"),
                )
        ...
        if g2p:
            ...
            if (
                X is not None
            ):  # On ne se déplace que sur le(s) axe(s) utilisé(s) par le palpage
