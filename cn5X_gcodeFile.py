...
class gcodeFile(QObject):
    ...
    def readFile(self, filePath: str):
        self.sig_log.emit(
            logSeverity.info.value, self.tr("Reading file: {}").format(filePath)
        )
        try:
            f = open(filePath, "r")
            lignes = f.readlines()
    ...
    def saveAs(self):
        ...
        if fName[0] != "":
            self.sig_log.emit(
                logSeverity.info.value, self.tr("saveAs({})").format(fName[0])
            )
    ...
    def saveFile(self, filePath: str = ""):
        ...
        self.sig_log.emit(
            logSeverity.info.value, self.tr("Saving file: {}").format(filePath)
        )
        try:
            f = open(filePath, "w")
            ...
            self.sig_log.emit(
                logSeverity.error.value, self.tr("Save file error: {}").format(filePath)
            )
            self.sig_log.emit(logSeverity.error.value, str(e))
    ...
    def showConfirmChangeLost(self):
        m = msgBox(
            ...
            info=self.tr(
                "If you do not save, any changes made since opening or the last save will be lost."
            ),
