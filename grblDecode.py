...
from PyQt5.QtCore import QCoreApplication, QEventLoop, QObject, pyqtSignal, pyqtSlot
...
class grblDecode(QObject):
    ...
    def __init__(self, ui, log, grbl: grblCom, beeper: cn5XBeeper, arretUrgence):
        ...
        self.__grblAlarm = [
            ...
            [
                6,
                self.tr("Homing fail"),
                self.tr("Homing fail. The active homing cycle was reset."),
            ],
            [
                7,
                self.tr("Homing fail"),
                self.tr("Homing fail. Safety door was opened during homing cycle."),
            ],
            [
                8,
                self.tr("Homing fail"),
    ...
    def decodeGrblStatus(self, grblOutput):
        if grblOutput[0] != "<" or grblOutput[-1] != ">":
            return self.tr(
                "grblDecode.py.decodeGrblStatus():error ! \n[{}] Incorrect status."
            ).format(grblOutput)

        # Affiche la chaine complette dans la barrs de status self.__statusText
        self.ui.statusBar.showMessage(
            "{} + {}".format(self.__grblCom.grblVersion(), grblOutput)
        )
        ...
        for D in tblDecode:
                ...
                if D != self.__etatMachine:
                    ...
                    if D == GRBL_STATUS_IDLE:
                        ...
                        if self.ui.btnStop.getButtonStatus():
                            self.ui.btnStop.setButtonStatus(False)
                        self.ui.lblEtat.setToolTip(
                            self.tr("Hold in-progress. Reset will throw an alarm.")
                        )
                    elif D == "Door:0":
                        self.ui.lblEtat.setToolTip(
                            self.tr("Door closed. Ready to resume.")
                        )
                    elif D == "Door:1":
                        self.ui.lblEtat.setToolTip(
                            self.tr(
                        ...
                        if not self.ui.btnStart.getButtonStatus():
                        ...
                        self.ui.lblEtat.setToolTip(
                            self.tr("Grbl Alarm! see Grbl communication.")
                        )
                    elif D == GRBL_STATUS_HOME:
                        self.ui.lblEtat.setToolTip(
                            self.tr("Grbl homing, wait for finish...")
                        )
                    else:
                        self.ui.lblEtat.setToolTip("")
            ...
            # Machine position MPos ($10=0 ou 2) ou WPos ($10=1 ou 3)?
                ...
                tblPos = D[5:].split(",")
                self.ui.lblPosX.setText("{:+0.3f}".format(float(tblPos[0])))
                self.ui.lblPosX.setToolTip(self.tr("Machine Position (MPos)."))
                self.ui.lblPosY.setText("{:+0.3f}".format(float(tblPos[1])))
                self.ui.lblPosY.setToolTip(self.tr("Machine Position (MPos)."))
                self.ui.lblPosZ.setText("{:+0.3f}".format(float(tblPos[2])))
                self.ui.lblPosZ.setToolTip(self.tr("Machine Position (MPos)."))
                if self.__nbAxis > 3:
                    self.ui.lblPosA.setText("{:+0.3f}".format(float(tblPos[3])))
                    self.ui.lblPosA.setToolTip(self.tr("Machine Position (MPos)."))
                else:
                    self.ui.lblPosA.setText("-")
                if self.__nbAxis > 4:
                    self.ui.lblPosB.setText("{:+0.3f}".format(float(tblPos[4])))
                    self.ui.lblPosB.setToolTip(self.tr("Machine Position (MPos)."))
                else:
                    self.ui.lblPosB.setText("-")
                if self.__nbAxis > 5:
                    self.ui.lblPosC.setText("{:+0.3f}".format(float(tblPos[5])))
                    self.ui.lblPosB.setToolTip(self.tr("Machine Position (MPos)."))
                else:
                ...
                tblPos = D[5:].split(",")
                self.ui.lblPosX.setText("{:+0.3f}".format(float(tblPos[0])))
                self.ui.lblPosX.setToolTip(self.tr("Working Position (WPos)."))
                self.ui.lblPosY.setText("{:+0.3f}".format(float(tblPos[1])))
                self.ui.lblPosY.setToolTip(self.tr("Working Position (WPos)."))
                self.ui.lblPosZ.setText("{:+0.3f}".format(float(tblPos[2])))
                self.ui.lblPosZ.setToolTip(self.tr("Working Position (WPos)."))
                if self.__nbAxis > 3:
                    self.ui.lblPosA.setText("{:+0.3f}".format(float(tblPos[3])))
                    self.ui.lblPosA.setToolTip(self.tr("Working Position (WPos)."))
                else:
                    self.ui.lblPosA.setText("-")
                if self.__nbAxis > 4:
                    self.ui.lblPosB.setText("{:+0.3f}".format(float(tblPos[4])))
                    self.ui.lblPosB.setToolTip(self.tr("Working Position (WPos)."))
                else:
                    self.ui.lblPosB.setText("-")
                if self.__nbAxis > 5:
                    self.ui.lblPosC.setText("{:+0.3f}".format(float(tblPos[5])))
                ...
                self.ui.progressBufferState.setToolTip(
                    "Buffer stat : " + tblValue[0] + "/" + tblValue[1]
                )
                ...
                if int(self.ui.lblAvancePourcent.text()[:-1]) != int(values[0]):
                    adjustFeedOverride(
                        int(values[0]),
                        int(self.ui.lblAvancePourcent.text()[:-1]),
                        self.__grblCom,
                    )
                # Avance rapide
                if values[1] == 25:
                ...
                if int(self.ui.lblBrochePourcent.text()[:-1]) != int(values[2]):
                    adjustSpindleOverride(
                        int(values[2]),
                        int(self.ui.lblBrochePourcent.text()[:-1]),
                        self.__grblCom,
                    )
    ...
    def decodeGrblResponse(self, grblOutput):
        if grblOutput == "ok":
            ...
            return self.tr("Grbl error number {}: {},\n{}").format(
                str(errNum), grblError[errNum][1], grblError[errNum][2]
            )
            ...
            return self.tr("Grbl Alarm number {}: {},\n{}").format(
                str(alarmNum),
                self.__grblAlarm[alarmNum][1],
                self.__grblAlarm[alarmNum][2],
            )
    ...
    def alarmMessage(self, alarmNum: int):
        return "ALARM:{}: {},\n{}".format(
            str(alarmNum), self.__grblAlarm[alarmNum][1], self.__grblAlarm[alarmNum][2]
        )
    ...
    def decodeGrblData(self, grblOutput):
        if grblOutput[:1] == "$":  # Setting output
            ...
            """Push Messages:"""
            if (
                grblOutput[1:4] in self.__validG5x
            ):  # ["G28", "G30", "G54","G55","G56","G57","G58","G59", "G92"]
                """
                ...
                if self.__getNextGCodeParams or self.__getNextProbe:
                    self.__getNextGCodeParams = (
                        False  # L'envoi du résultat de $# est complet
                    )
                    self.__getNextProbe = False
                    return grblOutput
                ...
                for S in tblGcodeParser:
                    if S in ["G54", "G55", "G56", "G57", "G58", "G59"]:
                        ...
                        for N, lbl in [
                            ...
                            if N == num:
                                lbl.setStyleSheet(
                                    "background-color:  rgb(0, 0, 63); color:rgb(248, 255, 192);"
                                )
                                font.setBold(True)
                                lbl.setFont(font)
                            else:
                                lbl.setStyleSheet(
                                    "background-color: rgb(248, 255, 192); color: rgb(0, 0, 63);"
                                )
                                font.setBold(False)
                                lbl.setFont(font)
                        ...
                        if S == "G17":
                            self.ui.lblPlan.setToolTip(self.tr(" Working plane = XY "))
                        if S == "G18":
                            self.ui.lblPlan.setToolTip(self.tr(" Working plane = ZX "))
                        if S == "G19":
                            self.ui.lblPlan.setToolTip(self.tr(" Working plane = YZ "))
                    elif S in ["G20", "G21"]:
                        self.ui.lblUnites.setText(S)
                        if S == "G20":
                            self.ui.lblUnites.setToolTip(self.tr(" Units = inches "))
                        if S == "G21":
                            self.ui.lblUnites.setToolTip(
                                self.tr(" Units = millimeters ")
                            )
                    elif S in ["G90", "G91"]:
                        self.ui.lblCoord.setText(S)
                        if S == "G90":
                            self.ui.lblCoord.setToolTip(
                                self.tr(" Absolute coordinates move ")
                            )
                        if S == "G91":
                            self.ui.lblCoord.setToolTip(
                                self.tr(" Relative coordinates move ")
                            )
                    elif S in [
                        "G0",
                        "G1",
                        ...
                        if S == "G0":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(" Rapid speed move. ")
                            )
                        if S == "G1":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Linear (straight line) motion at programed feed rate. "
                                )
                            )
                        if S == "G2":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Circular interpolation motion clockwise at programed feed rate. "
                                )
                            )
                        if S == "G3":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Circular interpolation motion counter-clockwise at programed feed rate. "
                                )
                            )
                        if S == "G38.2":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Probe: probe toward workpiece, stop on contact, signal error if failure. "
                                )
                            )
                        if S == "G38.3":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Probe: probe toward workpiece, stop on contact."
                                )
                            )
                        if S == "G38.4":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Probe: probe away from workpiece, stop on loss of contact, signal error if failure. "
                                )
                            )
                        if S == "G38.5":
                            self.ui.lblDeplacements.setToolTip(
                                self.tr(
                                    " Probe: probe away from workpiece, stop on loss of contact. "
                                )
                            )
                    elif S in ["G93", "G94"]:
                        self.ui.lblVitesse.setText(S)
                        if S == "G93":
                            self.ui.lblVitesse.setToolTip(
                                self.tr(" Inverse Time feed mode ")
                            )
                        if S == "G94":
                            self.ui.lblVitesse.setToolTip(
                                self.tr(" Units per minute feed mode ")
                            )
                    elif S in ["M3", "M4", "M5"]:
                        self.ui.lblBroche.setText(S)
                        if S == "M3":
                            self.ui.lblBroche.setToolTip(
                                self.tr(" Spindle clockwise at the S speed ")
                            )
                            if not self.ui.btnSpinM3.getButtonStatus():
                                self.ui.btnSpinM3.setButtonStatus(True)
                            if self.ui.btnSpinM4.getButtonStatus():
                                self.ui.btnSpinM4.setButtonStatus(False)
                            if self.ui.btnSpinM5.getButtonStatus():
                                self.ui.btnSpinM5.setButtonStatus(False)
                            if not self.ui.btnSpinM3.isEnabled():
                                self.ui.btnSpinM3.setEnabled(
                                    True
                                )  # Activation bouton M3
                            if self.ui.btnSpinM4.isEnabled():
                                self.ui.btnSpinM4.setEnabled(
                                    False
                                )  # Interdit un changement de sens de rotation direct
                        if S == "M4":
                            self.ui.lblBroche.setToolTip(
                                self.tr(" Spindle counter-clockwise at the S speed ")
                            )
                            if self.ui.btnSpinM3.getButtonStatus():
                                self.ui.btnSpinM3.setButtonStatus(False)
                            if not self.ui.btnSpinM4.getButtonStatus():
                                self.ui.btnSpinM4.setButtonStatus(True)
                            if self.ui.btnSpinM5.getButtonStatus():
                                self.ui.btnSpinM5.setButtonStatus(False)
                            if self.ui.btnSpinM3.isEnabled():
                                self.ui.btnSpinM3.setEnabled(
                                    False
                                )  # Interdit un changement de sens de rotation direct
                            if not self.ui.btnSpinM4.isEnabled():
                                self.ui.btnSpinM4.setEnabled(
                                    True
                                )  # Activation bouton M4
                        if S == "M5":
                            ...
                            if self.ui.btnSpinM4.getButtonStatus():
                                self.ui.btnSpinM4.setButtonStatus(False)
                            if not self.ui.btnSpinM5.getButtonStatus():
                        ...
                        if S == "M7":
                            ...
                            if not self.ui.btnFloodM7.getButtonStatus():
                                self.ui.btnFloodM7.setButtonStatus(True)
                            if self.ui.btnFloodM8.getButtonStatus():
                                self.ui.btnFloodM8.setButtonStatus(False)
                            if self.ui.btnFloodM9.getButtonStatus():
                                self.ui.btnFloodM9.setButtonStatus(False)
                            self.__etatArrosage = "M7"
                        if S == "M8":
                            self.ui.lblArrosage.setToolTip(
                                self.tr(" Flood coolant on ")
                            )
                            if self.ui.btnFloodM7.getButtonStatus():
                                self.ui.btnFloodM7.setButtonStatus(False)
                            if not self.ui.btnFloodM8.getButtonStatus():
                        ...
                        if S == "M9":
                            ...
                            if self.ui.btnFloodM8.getButtonStatus():
                                self.ui.btnFloodM8.setButtonStatus(False)
                            if not self.ui.btnFloodM9.getButtonStatus():
                                self.ui.btnFloodM9.setButtonStatus(True)
                            self.__etatArrosage = "M9"
                    elif S[:1] == "T":
                        self.ui.lblOutil.setText(S)
                        self.ui.lblOutil.setToolTip(
                            self.tr(" Tool number {}").format(S[1:])
                        )
                    elif S[:1] == "S":
                        self.ui.lblRotation.setText(S)
                        self.ui.lblRotation.setToolTip(
                            self.tr(
                        ...
                        self.ui.lblAvance.setToolTip(
                            self.tr(" Feed rate  = ").format(S[1:])
                        )
                    else:
                        return self.tr(
                            "Unknown G-code Parser status in {} : {}"
                        ).format(grblOutput, S)
            ...
            if grblOutput != "":
                self.log(
                    ...
                    self.tr("Not decoded Grbl reply : [{}]").format(grblOutput),
                )
            return grblOutput
    ...
    def grblSetting(self, num):
        ...
        grblSettingsCodes = {
            0: [
                self.tr("Step pulse time"),
                self.tr("microseconds"),
                self.tr("Sets time length per step (Minimum 3usec)."),
            ],
            1: [
                self.tr("Step idle delay"),
            ...
            2: [
                self.tr("Step pulse invert"),
                self.tr("mask"),
                self.tr("Inverts the step signal. Set axis bit to invert (00000ZYX)."),
            ],
            3: [
                self.tr("Step direction invert"),
                self.tr("mask"),
                self.tr(
            ...
            4: [
                self.tr("Invert step enable pin"),
                self.tr("boolean"),
                self.tr("Inverts the stepper driver enable pin signal."),
            ],
            5: [
                self.tr("Invert limit pins"),
                self.tr("boolean"),
                self.tr("Inverts the all of the limit input pins."),
            ],
            6: [
                self.tr("Invert probe pin"),
                self.tr("boolean"),
                self.tr("Inverts the probe input pin signal."),
            ],
            10: [
                self.tr("Status report options"),
                self.tr("mask"),
                self.tr("Alters data included in status reports."),
            ],
            11: [
                self.tr("Junction deviation"),
                self.tr("millimeters"),
                self.tr(
            ...
            22: [
                self.tr("Homing cycle enable"),
                self.tr("boolean"),
                self.tr("Enables homing cycle. Requires limit switches on all axes."),
            ],
            23: [
                self.tr("Homing direction invert"),
                self.tr("mask"),
                self.tr(
            ...
            24: [
                self.tr("Homing locate feed rate"),
                self.tr("units (millimeters or degres)/min"),
                self.tr(
                    "Feed rate to slowly engage limit switch to determine its location accurately."
                ),
            ],
            25: [
                self.tr("Homing search seek rate"),
                self.tr("units (millimeters or degres)/min"),
                self.tr(
            ...
            26: [
                self.tr("Homing switch debounce delay"),
                self.tr("milliseconds"),
                self.tr(
            ...
            27: [
                self.tr("Homing switch pull-off distance"),
                self.tr("millimeters"),
                self.tr(
            ...
            30: [
                self.tr("Maximum spindle speed"),
                self.tr("RPM"),
                self.tr("Maximum spindle speed. Sets PWM to 100% duty cycle."),
            ],
            31: [
                self.tr("Minimum spindle speed"),
                self.tr("RPM"),
                self.tr(
            ...
            100: [
                self.tr("1st axis travel resolution"),
                self.tr("step/unit"),
                self.tr(
                    "1st axis travel resolution in steps per unit (millimeter or degre)."
                ),
            ],
            101: [
                self.tr("2nd axis travel resolution"),
                self.tr("step/unit"),
                self.tr(
                    "2nd axis travel resolution in steps per unit (millimeter or degre)."
                ),
            ],
            102: [
                self.tr("3rd axis travel resolution"),
                self.tr("step/unit"),
                self.tr(
                    "3rd axis travel resolution in steps per unit (millimeter or degre)."
                ),
            ],
            103: [
                self.tr("4th axis travel resolution"),
                self.tr("step/unit"),
                self.tr(
                    "4th axis travel resolution in steps per unit (millimeter or degre)."
                ),
            ],
            104: [
                self.tr("5th axis travel resolution"),
                self.tr("step/unit"),
                self.tr(
                    "5th axis travel resolution in steps per unit (millimeter or degre)."
                ),
            ],
            105: [
                self.tr("6th axis travel resolution"),
                self.tr("step/unit"),
                self.tr(
                    "6th axis travel resolution in steps per unit (millimeter or degre)."
                ),
            ],
            110: [
                self.tr("1st axis maximum rate"),
                self.tr("unit/min"),
                self.tr("1st axis maximum rate. Used as G0 rapid rate."),
            ],
            111: [
                self.tr("2nd axis maximum rate"),
                self.tr("unit/min"),
                self.tr("2nd axis maximum rate. Used as G0 rapid rate."),
            ],
            112: [
                self.tr("3rd axis maximum rate"),
                self.tr("unit/min"),
                self.tr("3rd axis maximum rate. Used as G0 rapid rate."),
            ],
            113: [
                self.tr("4th axis maximum rate"),
                self.tr("unit/min"),
                self.tr("4th axis maximum rate. Used as G0 rapid rate"),
            ],
            114: [
                self.tr("5th axis maximum rate"),
                self.tr("unit/min"),
                self.tr("5th axis maximum rate. Used as G0 rapid rate"),
            ],
            115: [
                self.tr("6th axis maximum rate"),
                self.tr("unit/min"),
                self.tr("6th axis maximum rate. Used as G0 rapid rate"),
            ],
            120: [
                self.tr("1st axis acceleration"),
                self.tr("unit/sec^2"),
                self.tr(
                    "1st axis acceleration. Used for motion planning to not exceed motor torque and lose steps."
                ),
            ],
            121: [
                self.tr("2nd axis acceleration"),
                self.tr("unit/sec^2"),
                self.tr(
                    "2nd axis acceleration. Used for motion planning to not exceed motor torque and lose steps."
                ),
            ],
            122: [
                self.tr("3rd axis acceleration"),
                self.tr("unit/sec^2"),
                self.tr(
                    "3rd axis acceleration. Used for motion planning to not exceed motor torque and lose steps."
                ),
            ],
            123: [
                self.tr("4th axis acceleration"),
                self.tr("unit/sec^2"),
                self.tr(
                    "4th axis acceleration. Used for motion planning to not exceed motor torque and lose steps."
                ),
            ],
            124: [
                self.tr("5th axis acceleration"),
                self.tr("unit/sec^2"),
                self.tr(
                    "5th axis acceleration. Used for motion planning to not exceed motor torque and lose steps."
                ),
            ],
            125: [
                self.tr("6th axis acceleration"),
                self.tr("unit/sec^2"),
                self.tr(
                    "6th axis acceleration. Used for motion planning to not exceed motor torque and lose steps."
                ),
            ],
            130: [
                self.tr("1st axis maximum travel"),
                self.tr("unit (millimeters or degres)"),
                self.tr(
                    "Maximum 1st axis travel distance from homing switch. Determines valid machine space for soft-limits and homing search distances."
                ),
            ],
            131: [
                self.tr("2nd axis maximum travel"),
                self.tr("unit (millimeters or degres)"),
                self.tr(
                    "Maximum 2nd axis travel distance from homing switch. Determines valid machine space for soft-limits and homing search distances."
                ),
            ],
            132: [
                self.tr("3rd axis maximum travel"),
                self.tr("unit (millimeters or degres)"),
                self.tr(
                    "Maximum 3rd axis travel distance from homing switch. Determines valid machine space for soft-limits and homing search distances."
                ),
            ],
            133: [
                self.tr("4th axis maximum travel"),
                self.tr("unit (millimeters or degres)"),
                self.tr(
                    "Maximum 4th axis travel distance from homing switch. Determines valid machine space for soft-limits and homing search distances."
                ),
            ],
            134: [
                self.tr("5th axis maximum travel"),
                self.tr("unit (millimeters or degres)"),
                self.tr(
                    "Maximum 5th axis travel distance from homing switch. Determines valid machine space for soft-limits and homing search distances."
                ),
            ],
            135: [
                self.tr("6th axis maximum travel"),
                self.tr("unit (millimeters or degres)"),
                self.tr(
                    "Maximum 6th axis travel distance from homing switch. Determines valid machine space for soft-limits and homing search distances."
                ),
            ],
        }
    ...
    def updateAxisDefinition(self):
        ...
        self.ui.mnuG5X_reset.setText(
            "Turn off and reset G{} offsets of all axis".format(self.__G5actif)
        )
        self.ui.mnuG5X_origine_0.setText(
            "Place the G{} origin of all axis here".format(self.__G5actif)
        )
        self.ui.mnuG5X_origine_1.setText(
            "Place the G{} origin of axis {} here".format(
                self.__G5actif, self.__axisNames[0]
            )
        )
        self.ui.mnuG5X_origine_2.setText(
            "Place the G{} origin of axis {} here".format(
                self.__G5actif, self.__axisNames[1]
            )
        )
        self.ui.mnuG5X_origine_3.setText(
            "Place the G{} origin of axis {} here".format(
                self.__G5actif, self.__axisNames[2]
            )
        )
        ...
        if self.__nbAxis > 3:
            ...
            self.ui.mnuG5X_origine_4.setText(
                "Place the G{} origin of axis {} here".format(
                    self.__G5actif, self.__axisNames[3]
                )
            )
            ...
            self.ui.mnuG5X_origine_4.setText(
                "Place the G{} origin of axis - here".format(self.__G5actif)
            )
        ...
        if self.__nbAxis > 4:
            ...
            self.ui.mnuG5X_origine_5.setText(
                "Place the G{} origin of axis {} here".format(
                    self.__G5actif, self.__axisNames[4]
                )
            )
        ...
        if self.__nbAxis > 5:
            ...
            self.ui.mnuG5X_origine_6.setText(
                "Place the G{} origin of axis {} here".format(
                    self.__G5actif, self.__axisNames[5]
                )
            )
