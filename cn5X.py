...
class winMain(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.__settings = QSettings(
            QSettings.NativeFormat, QSettings.UserScope, ORG_NAME, APP_NAME
        )
        ...
        parser.add_argument(
            ...
            "--connect",
            action="store_true",
            help=self.tr("Connect the serial port"),
        )
        parser.add_argument("-f", "--file", help=self.tr("Load the GCode file"))
        parser.add_argument(
            "-l", "--lang", help=self.tr("Define the interface language")
        )
        ...
        parser.add_argument(
            ...
            "--noUrgentStop",
            action="store_true",
            help=self.tr("Unlock urgent stop"),
        )
        ...
        self.logGrbl = (
            self.ui.txtGrblOutput
        )  # Tous les messages de Grbl seront rediriges dans le widget txtGrblOutput
        self.logCn5X = (
            self.ui.txtConsoleOutput
        )  # Tous les messages applicatif seront rediriges dans le widget txtConsoleOutput
        self.logDebug = self.ui.txtDebugOutput  # Message debug de Grbl

        self.logGrbl.document().setMaximumBlockCount(
            2000
        )  # Limite la taille des logs a 2000 lignes
        self.logCn5X.document().setMaximumBlockCount(
            2000
        )  # Limite la taille des logs a 2000 lignes
        self.logDebug.document().setMaximumBlockCount(
            2000
        )  # Limite la taille des logs a 2000 lignes
        self.ui.qtabConsole.setCurrentIndex(
            CN5X_TAB_LOG
        )  # Active le tab de la log cn5X++
        ...
        self.__decode = grblDecode(
            self.ui, self.log, self.__grblCom, self.__beeper, arretUrgence
        )
        ...
        app.setStyleSheet(
            "QToolTip { background-color: rgb(248, 255, 192); color: rgb(0, 0, 63); }"
        )
        ...
        self.ui.btnUrgence.pressed.connect(
            self.on_arretUrgence
        )  # Evenements du bouton d'arret d'urgence
        self.ui.cmbPort.currentIndexChanged.connect(
            self.on_cmbPort_changed
        )  # un clic sur un element de la liste appellera la methode 'on_cmbPort_changed'

        self.ui.mnuBar.hovered.connect(
            self.on_mnuBar
        )  # Connexions des routines du menu application
        ...
        # Sous-menu G28/G30
        self.ui.mnuPredefinedLocations.aboutToShow.connect(
            self.on_mnuPredefinedLocations
        )
        ...
        # Menu d'aide
        self.ui.mnuHelpProbe_single_axis.triggered.connect(
            lambda: self.on_mnuHelpProbe(MENU_SINGLE_AXIS)
        )
        self.ui.mnuHelpProbe_inside_corner.triggered.connect(
            lambda: self.on_mnuHelpProbe(MENU_INSIDE_CORNER)
        )
        self.ui.mnuHelpProbe_outside_corner.triggered.connect(
            lambda: self.on_mnuHelpProbe(MENU_OUTSIDE_CORNER)
        )
        self.ui.mnuHelpProbe_inside_center.triggered.connect(
            lambda: self.on_mnuHelpProbe(MENU_INSIDE_CENTER)
        )
        self.ui.mnuHelpProbe_outside_center.triggered.connect(
            lambda: self.on_mnuHelpProbe(MENU_OUTSIDE_CENTER)
        )
        self.ui.mnuA_propos.triggered.connect(self.on_mnuA_propos)

        self.ui.btnRefresh.clicked.connect(
            self.populatePortList
        )  # Refresh de la liste des ports serie
        self.ui.btnConnect.clicked.connect(
            self.action_btnConnect
        )  # un clic sur le bouton "(De)Connecter" appellera la methode 'action_btnConnect'
        self.ui.btnSend.pressed.connect(
            self.sendCmd
        )  # Bouton d'envoi de commandes unitaires
        self.ui.txtGCode.setValidator(
            self.ucase
        )  # Force la saisie des GCodes en majuscules
        self.ui.txtGCode.returnPressed.connect(
            self.sendCmd
        )  # Meme fonction par la touche entree que le bouton d'envoi
        self.ui.txtGCode.textChanged.connect(
            self.txtGCode_on_Change
        )  # Analyse du champ de saisie au fur et a mesure de son edition
        ...
        self.ui.rbRapid025.clicked.connect(
            lambda: self.__grblCom.realTimePush(REAL_TIME_RAPID_25_POURCENT)
        )
        self.ui.rbRapid050.clicked.connect(
            lambda: self.__grblCom.realTimePush(REAL_TIME_RAPID_50_POURCENT)
        )
        self.ui.rbRapid100.clicked.connect(
            lambda: self.__grblCom.realTimePush(REAL_TIME_RAPID_100_POURCENT)
        )
        ...
        self.ui.gcodeTable.customContextMenuRequested.connect(
            self.on_gcodeTableContextMenu
        )
        QtWidgets.QShortcut(
            QtCore.Qt.Key_F7,
            self.ui.gcodeTable,
            activated=self.on_GCodeTable_key_F7_Pressed,
        )
        QtWidgets.QShortcut(
            QtCore.Qt.Key_F8,
            self.ui.gcodeTable,
            activated=self.on_GCodeTable_key_F8_Pressed,
        )
        self.ui.dialAvance.customContextMenuRequested.connect(
            self.on_dialAvanceContextMenu
        )
        self.ui.dialBroche.customContextMenuRequested.connect(
            self.on_dialBrocheContextMenu
        )
        self.ui.lblLblPosX.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(0)
        )
        self.ui.lblLblPosY.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(1)
        )
        self.ui.lblLblPosZ.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(2)
        )
        self.ui.lblLblPosA.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(3)
        )
        self.ui.lblLblPosB.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(4)
        )
        self.ui.lblLblPosC.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(5)
        )
        self.ui.lblPosX.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(0)
        )
        self.ui.lblPosY.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(1)
        )
        self.ui.lblPosZ.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(2)
        )
        self.ui.lblPosA.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(3)
        )
        self.ui.lblPosB.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(4)
        )
        self.ui.lblPosC.customContextMenuRequested.connect(
            lambda: self.on_lblPosContextMenu(5)
        )
        self.ui.lblPlan.customContextMenuRequested.connect(self.on_lblPlanContextMenu)
        self.ui.lblUnites.customContextMenuRequested.connect(
            self.on_lblUnitesContextMenu
        )
        self.ui.lblCoord.customContextMenuRequested.connect(self.on_lblCoordContextMenu)
        self.ui.lblG54.customContextMenuRequested.connect(
            lambda: self.on_lblGXXContextMenu(1)
        )
        self.ui.lblG55.customContextMenuRequested.connect(
            lambda: self.on_lblGXXContextMenu(2)
        )
        self.ui.lblG56.customContextMenuRequested.connect(
            lambda: self.on_lblGXXContextMenu(3)
        )
        self.ui.lblG57.customContextMenuRequested.connect(
            lambda: self.on_lblGXXContextMenu(4)
        )
        self.ui.lblG58.customContextMenuRequested.connect(
            lambda: self.on_lblGXXContextMenu(5)
        )
        self.ui.lblG59.customContextMenuRequested.connect(
            lambda: self.on_lblGXXContextMenu(6)
        )
        ...
        # Onglet probe XY
        self.ui.dsbToolDiameter.valueChanged.connect(
            self.on_dsbToolDiameterValueChanged
        )
    ...
    def populatePortList(self):
        ...
        if len(ports) > 0:
            ...
            m = msgBox(
                ...
                info=self.tr(
                    "{} could not find a serial port allowing to communicate with grbl."
                ).format(sys.argv[0]),
    ...
    def setProbeButtonsToolTip(self):
        ...
        if self.ui.rbtProbeInsideXY.isChecked():
            ...
            self.ui.btnProbeXY_0.setToolTip(
                self.tr(
                    "Run probe in X+, X-, Y+ and Y- direction to find inside center."
                )
            )
            self.ui.btnProbeXY_1.changeIcon(":/cn5X/images/btnProbeInX-Y+.svg")
            self.ui.btnProbeXY_1.setToolTip(
                self.tr("Run probe in X- and Y+ direction.")
            )
            ...
            self.ui.btnProbeXY_3.setToolTip(
                self.tr("Run probe in X+ and Y+ direction.")
            )
            ...
            self.ui.btnProbeXY_5.setToolTip(
                self.tr("Run probe in X+ and Y- direction.")
            )
            ...
            self.ui.btnProbeXY_7.setToolTip(
                self.tr("Run probe in X- and Y- direction.")
            )
            ...
            self.ui.btnProbeXY_0.setToolTip(
                self.tr(
                    "Run probe in X+, X-, Y+ and Y- direction to find outside center."
                )
            )
    ...
    def setEnableDisableGroupes(self):
        ...
        if not self.__connectionStatus:
            ...
            self.ui.btnUrgence.setToolTip(
                self.tr("Double click to\nunlock the emergency stop")
            )
    ...
    def on_mnuPreferences(self):
        self.ui.mnuConfirm_Go_to_G28.setChecked(
            not self.__settings.value("dontConfirmG28", False, type=bool)
        )
        self.ui.mnuConfirm_Go_to_G30.setChecked(
            not self.__settings.value("dontConfirmG30", False, type=bool)
        )
        self.ui.mnuConfirm_define_G28.setChecked(
            not self.__settings.value("dontConfirmG28.1", False, type=bool)
        )
        self.ui.mnuConfirm_define_G30.setChecked(
            not self.__settings.value("dontConfirmG30.1", False, type=bool)
        )

    @pyqtSlot()
    def on_mnuConfirm_Go_to_G28(self):
        self.__settings.setValue(
            "dontConfirmG28", not self.ui.mnuConfirm_Go_to_G28.isChecked()
        )

    @pyqtSlot()
    def on_mnuConfirm_Go_to_G30(self):
        self.__settings.setValue(
            "dontConfirmG30", not self.ui.mnuConfirm_Go_to_G30.isChecked()
        )

    @pyqtSlot()
    def on_mnuConfirm_define_G28(self):
        self.__settings.setValue(
            "dontConfirmG28.1", not self.ui.mnuConfirm_define_G28.isChecked()
        )

    @pyqtSlot()
    def on_mnuConfirm_define_G30(self):
        self.__settings.setValue(
            "dontConfirmG30.1", not self.ui.mnuConfirm_define_G30.isChecked()
        )
    ...
    def closeEvent(self, event):
        ...
        if not self.__gcodeFile.closeFile():
            ...
            event.setAccepted(
                False
            )  # True accepte la fermeture, False annule la fermeture
        else:
    ...
    def on_mnuRestoreG92(self):
        ...
        if (axisNamesNbSav != self.__nbAxis) or (
            axisNamesSaved != "".join(self.__axisNames)
        ):
            m = msgBox(
                title=self.tr("Error !"),
                text=self.tr(
                    "Saved axis definition is not identical to the current one!"
                ),
                info="[AXS:{}:{}] != [AXS:{}:{}]".format(
                    axisNamesNbSav,
                    axisNamesSaved,
                    self.__nbAxis,
                    "".join(self.__axisNames),
                ),
                icon=msgIconList.Critical,
                detail=self.tr(
                    "Can't restore G92 offsets if the current axis definition (axis number and names) is not the same as the saved one."
                ),
                stdButton=msgButtonList.Close,
            )
            m.afficheMsg()
        ...
        if m.afficheMsg() == msgButtonList.Yes:
            ...
            for i in range(6):
                ...
                if actualValue is not None:
                    if self.__axisNames[i] not in axesTraites:
                        newAxisWpos = (
                            self.__decode.getMpos(i)
                            - newValue[i]
                            - self.__decode.getOffsetG5x(i)
                        )
    ...
    def on_mnuJog_to(self):
        ...
        if self.dlgJog is None:
            self.dlgJog = dlgJog(
                self.__grblCom, self.__decode, self.__nbAxis, self.__axisNames
            )
    ...
    def on_gotoG28(self):
        ...
        dlg = dlgG28_30_1(
            "G28", self.__grblCom, self.__decode, self.__nbAxis, self.__axisNames
        )
        dlg.setParent(self)
        dlg.showDialog()
    ...
    def on_gotoG30(self):
        ...
        dlg = dlgG28_30_1(
            "G30", self.__grblCom, self.__decode, self.__nbAxis, self.__axisNames
        )
        dlg.setParent(self)
        dlg.showDialog()
        # On laisse le temps à Grbl de commencer
    ...
    def on_mnuDefineG28(self):
        ...
        dlg = dlgG28_30_1(
            "G28.1", self.__grblCom, self.__decode, self.__nbAxis, self.__axisNames
        )
        dlg.setParent(self)
        dlg.showDialog()
    ...
    def on_mnuDefineG30(self):
        ...
        dlg = dlgG28_30_1(
            "G30.1", self.__grblCom, self.__decode, self.__nbAxis, self.__axisNames
        )
        dlg.setParent(self)
        dlg.showDialog()
    ...
    def on_qtabMain_currentChanged(self, tabIndex):
        ...
        if self.__currentQTabMainIndex != tabIndex:
            if tabIndex == CN5X_TAB_MAIN:
                ...
                self.ui.dsbToolDiameter.setValue(
                    self.__settings.value(
                        "Probe/ToolDiameter", DEFAULT_TOOL_DIAMATER, type=float
                    )
                )
                self.ui.dsbDistanceXY.setValue(
                    self.__settings.value(
                        "Probe/DistanceXY", DEFAULT_PROBE_DISTANCE, type=float
                    )
                )
                self.ui.dsbClearanceXY.setValue(
                    self.__settings.value(
                        "Probe/ClearanceXY", DEFAULT_CLEARANCE_XY, type=float
                    )
                )
                self.ui.dsbClearanceZ.setValue(
                    self.__settings.value(
                        "Probe/ClearanceZ", DEFAULT_CLEARANCE_Z, type=float
                    )
                )
                self.ui.dsbFeedRateXY.setValue(
                    self.__settings.value(
                        "Probe/FeedRateXY", DEFAULT_PROBE_FEED_RATE, type=float
                    )
                )
                self.ui.chkSeekXY.setChecked(
                    self.__settings.value(
                        "Probe/DoubleProbeXY", DEFAULT_PROBE_SEEK, type=bool
                    )
                )
                self.on_chkSeekXY()
                self.ui.dbsSeekRateXY.setValue(
                    self.__settings.value(
                        "Probe/SeekRateXY", DEFAULT_PROBE_SEEK_RATE, type=float
                    )
                )
                self.ui.dsbPullOffXY.setValue(
                    self.__settings.value(
                        "Probe/PullOffXY",
                        DEFAULT_PROBE_PULL_OFF_DISTANCE_XY,
                        type=float,
                    )
                )
                self.ui.gbMoveAfterXY.setChecked(
                    self.__settings.value(
                        "Probe/MoveAfterXY", DEFAULT_PROBE_MOVE_AFTER_XY, type=bool
                    )
                )
                self.ui.rbtMove2PointAfterXY.setChecked(
                    self.__settings.value(
                        "Probe/go2PointXY", DEFAULT_PROBE_GO_2_POINT_AFTER_XY, type=bool
                    )
                )
                self.ui.rbtRetractAfterXY.setChecked(
                    self.__settings.value(
                        "Probe/RetractAfterXY",
                        DEFAULT_PROBE_RETRACT_AFTER_XY,
                        type=bool,
                    )
                )
                self.ui.dsbRetractXY.setValue(
                    self.__settings.value(
                        "Probe/RetractDistanceXY",
                        DEFAULT_PROBE_RETRACT_DISTANCE_AFTER_XY,
                        type=float,
                    )
                )

            elif tabIndex == CN5X_TAB_PROBE_Z:
                self.ui.dsbDistanceZ.setValue(
                    self.__settings.value(
                        "Probe/DistanceZ", DEFAULT_PROBE_DISTANCE, type=float
                    )
                )
                self.ui.dsbFeedRateZ.setValue(
                    self.__settings.value(
                        "Probe/FeedRateZ", DEFAULT_PROBE_FEED_RATE, type=float
                    )
                )
                self.ui.chkSeekZ.setChecked(
                    self.__settings.value(
                        "Probe/DoubleProbeZ", DEFAULT_PROBE_SEEK, type=bool
                    )
                )
                self.on_chkSeekZ()
                self.ui.dbsSeekRateZ.setValue(
                    self.__settings.value(
                        "Probe/SeekRateZ", DEFAULT_PROBE_SEEK_RATE, type=float
                    )
                )
                self.ui.dsbPullOffZ.setValue(
                    self.__settings.value(
                        "Probe/PullOffZ", DEFAULT_PROBE_PULL_OFF_DISTANCE_Z, type=float
                    )
                )
                self.ui.gbMoveAfterZ.setChecked(
                    self.__settings.value(
                        "Probe/MoveAfterZ", DEFAULT_PROBE_MOVE_AFTER_Z, type=bool
                    )
                )
                self.ui.rbtMove2PointAfterZ.setChecked(
                    self.__settings.value(
                        "Probe/go2PointZ", DEFAULT_PROBE_GO_2_POINT_AFTER_Z, type=bool
                    )
                )
                self.ui.rbtRetractAfterZ.setChecked(
                    self.__settings.value(
                        "Probe/RetractAfterZ", DEFAULT_PROBE_RETRACT_AFTER_Z, type=bool
                    )
                )
                self.ui.dsbRetractZ.setValue(
                    self.__settings.value(
                        "Probe/RetractDistanceZ",
                        DEFAULT_PROBE_RETRACT_DISTANCE_AFTER_Z,
                        type=float,
                    )
                )
                self.ui.rbtDefineOriginZ_G54.setChecked(
                    self.__settings.value(
                        "Probe/DefineOriginZ_G54",
                        DEFAULT_PROBE_ORIGINE_G54_Z,
                        type=bool,
                    )
                )
                self.ui.rbtDefineOriginZ_G92.setChecked(
                    self.__settings.value(
                        "Probe/DefineOriginZ_G92",
                        DEFAULT_PROBE_ORIGINE_G92_Z,
                        type=bool,
                    )
                )
                self.ui.dsbOriginOffsetZ.setValue(
                    self.__settings.value(
                        "Probe/OriginOffsetZ",
                        DEFAULT_PROBE_ORIGINE_OFFSET_Z,
                        type=float,
                    )
                )
                self.ui.dsbToolLengthSensorX.setValue(
                    self.__settings.value(
                        "Probe/ToolSensorPositionX",
                        DEFAULT_TOOLSENSOR_POSITION_X,
                        type=float,
                    )
                )
                self.ui.dsbToolLengthSensorY.setValue(
                    self.__settings.value(
                        "Probe/ToolSensorPositionY",
                        DEFAULT_TOOLSENSOR_POSITION_Y,
                        type=float,
                    )
                )
    ...
    def on_btnProbeZ(self):
        ...
        try:
            if doubleProbe:
                ...
                self.__probeResult = self.__probe.g38(
                    P=3, F=probeSeekRate, Z=-probeDistance, g2p=False
                )
                # On mémorise le résultat
                self.ui.lblLastProbZ.setText(
                    "{:+0.3f}".format(float(self.__probeResult.getAxisByName("Z")))
                )
            ...
            self.__probeResult = self.__probe.g38(
                P=3, F=probeFeedRate, Z=-fineProbeDistance, g2p=go2point
            )
            ...
            self.log(
                ...
                self.tr(
                    "on_btnProbeZ(): The requested axis ({}) is not in the axis list of this machine"
                ).format(e),
            )
            pass
            ...
            self.log(
                logSeverity.error.value,
                self.tr("on_btnProbeZ(): {} no response from probe").format(e),
            )
            pass
            ...
            self.log(
                logSeverity.error.value,
                self.tr("on_btnProbeZ(): {} Probe error").format(e),
            )
            pass
            ...
            self.log(
                logSeverity.error.value,
                self.tr(
                    "on_btnProbeZ(): F Speed undefined or less or equal to zero"
                ).format(e),
            )
            pass
        ...
        self.__settings.setValue(
            "Probe/go2PointZ", self.ui.rbtMove2PointAfterZ.isChecked()
        )
        self.__settings.setValue(
            "Probe/RetractAfterZ", self.ui.rbtRetractAfterZ.isChecked()
        )
    ...
    def on_btnG49(self):
        ...
        if not self.__initialProbeZ:
            self.log(
                logSeverity.error.value,
                self.tr(
                    "on_btnG49(): No initial Z probe result, can't get initial tool length probe!"
                ),
            )
            m = msgBox(
                title=self.tr("Error !"),
                text=self.tr(
                    "No initial Z probe result, can't get initial tool length probe!"
                ),
                info=self.tr("There was no Z probing previously performed.."),
                icon=msgIconList.Critical,
                detail=self.tr(
                    "You must first perform a Z probing of the initial tool to initialize its length."
                ),
                stdButton=msgButtonList.Close,
            )
            m.afficheMsg()
    ...
    def on_btnG43_1(self):
        ...
        if not self.__initialToolLenght:
            self.log(
                logSeverity.error.value,
                self.tr(
                    "on_btnG43_1(): No initial tool length, can't calculate length offset!"
                ),
            )
            m = msgBox(
                title=self.tr("Error !"),
    ...
    def on_btnSetOriginZ(self):
        ...
        if self.ui.mnu_MPos.isChecked():
            ...
            positionZ = float(self.ui.lblPosZ.text().replace(" ", "")) + float(
                self.ui.lblWcoZ.text().replace(" ", "")
            )
        # Offset à ajouter
        ...
        self.__settings.setValue(
            "Probe/DefineOriginZ_G54", self.ui.rbtDefineOriginZ_G54.isChecked()
        )
        self.__settings.setValue(
            "Probe/DefineOriginZ_G92", self.ui.rbtDefineOriginZ_G92.isChecked()
        )
        self.__settings.setValue(
            "Probe/OriginOffsetZ", self.ui.dsbOriginOffsetZ.value()
        )
    ...
    def on_btnProbeXY(self, btnNum: int):
        ...
        def probeXplus(length=probeDistance):
            if doubleProbeXY:
                ...
                if probeInside:
                    ...
                    self.ui.qfProbeResultXmax.setText(
                        "{:+0.3f}".format(self.__xMaxValue)
                    )
                    self.ui.qfProbeResultXmax.setStyleSheet("color: #000020;")
                    self.__xMax = True
                else:
                    self.__xMinValue = float(pr.getAxisByName("X"))
                    self.ui.qfProbeResultXmin.setText(
                        "{:+0.3f}".format(self.__xMinValue)
                    )
        ...
        def probeYplus(length=probeDistance):
            if doubleProbeXY:
                ...
                if probeInside:
                    ...
                    self.ui.qfProbeResultYmax.setText(
                        "{:+0.3f}".format(self.__yMaxValue)
                    )
                    self.ui.qfProbeResultYmax.setStyleSheet("color: #000020;")
                    self.__yMax = True
                else:
                    self.__yMinValue = float(pr.getAxisByName("Y"))
                    self.ui.qfProbeResultYmin.setText(
                        "{:+0.3f}".format(self.__yMinValue)
                    )
        ...
        try:
            if (
                btnNum == 4 and probeInside or btnNum == 8 and probeOutside
            ):  # inside X+, outside X+
                probeXplus()
                if retractAfterXY:
                ...
                btnNum == 8 and probeInside or btnNum == 4 and probeOutside
            ):  # inside X-, outside X-
                probeXmoins()
                if retractAfterXY:
                    self.__grblCom.gcodePush("G0X{}".format(retractXY))
                ...
                btnNum == 2 and probeInside or btnNum == 6 and probeOutside
            ):  # inside Y+, outside Y+
                probeYplus()
                if retractAfterXY:
                ...
                btnNum == 6 and probeInside or btnNum == 2 and probeOutside
            ):  # inside Y-, outside Y-
                probeYmoins()
                if retractAfterXY:
                    self.__grblCom.gcodePush("G0Y{}".format(retractXY))
                ...
                self.__grblCom.gcodePush(
                    "G0X{:+0.3f}Y{:+0.3f}".format(-clearanceXY, -probeDistance)
                )
                ...
                self.__grblCom.gcodePush(
                    "G0X{:+0.3f}Y{:+0.3f}".format(
                        clearanceXY - probeDistance + (toolDiameter / 2),
                        probeDistance - clearanceXY,
                    )
                )
                ...
                self.__grblCom.gcodePush(
                    "G0X{:+0.3f}Y{:+0.3f}".format(
                        probeDistance, clearanceXY + (toolDiameter / 2)
                    )
                )
                self.ui.chkAddOffsetX.setChecked(True)
                self.ui.chkAddOffsetY.setChecked(True)
                ...
                self.__grblCom.gcodePush("G0Z{:+0.3f}".format(-clearanceZ))
                self.__grblCom.gcodePush(
                    "G0X{:+0.3f}".format(-probeDistance + clearanceXY)
                )
                probeXmoins(clearanceXY)
                self.__grblCom.gcodePush(
                    "G0X{:+0.3f}".format(
                        (2 * probeDistance) - clearanceXY - (toolDiameter / 2)
                    )
                )
                probeXplus(clearanceXY)
                ...
                self.__grblCom.gcodePush(
                    "G0Y{:+0.3f}".format(-probeDistance + clearanceXY)
                )
                probeYmoins(clearanceXY)
                self.__grblCom.gcodePush(
                    "G0Y{:+0.3f}".format(
                ...
                self.__grblCom.gcodePush(
                    "G0X{:+0.3f}".format(
                        2 * (probeDistance + clearanceXY) + toolDiameter
                    )
                )
                self.__grblCom.gcodePush("G0Z{:+0.3f}".format(-clearanceZ))
            ...
            self.log(
                logSeverity.error.value,
                self.tr(
                    "on_btnProbeXY(): The requested axis ({}) is not in the axis list of this machine"
                ).format(e),
            )
            pass
            ...
            self.log(
                logSeverity.error.value,
                self.tr("on_btnProbeXY(): {} no response from probe").format(e),
            )
            pass
            ...
            self.log(
                logSeverity.error.value,
                self.tr("on_btnProbeXY(): {} Probe error").format(e),
            )
            pass
            ...
            self.log(
                logSeverity.error.value,
                self.tr(
                    "on_btnProbeXY(): F Speed undefined or less or equal to zero"
                ).format(e),
            )
            pass
        ...
        self.__settings.setValue(
            "Probe/go2PointXY", self.ui.rbtMove2PointAfterXY.isChecked()
        )
        self.__settings.setValue(
            "Probe/RetractAfterXY", self.ui.rbtRetractAfterXY.isChecked()
        )
        self.__settings.setValue(
            "Probe/RetractDistanceXY", self.ui.dsbRetractXY.value()
        )

    def calculateCenterXY(self):
        if self.__xMin and self.__xMax:
            self.ui.qfProbeResultXcenter.setText(
                "{:+0.3f}".format(
                    (
                        float(self.ui.qfProbeResultXmax.text())
                        + float(self.ui.qfProbeResultXmin.text())
                    )
                    / 2
                )
            )
            self.ui.qfProbeResultXcenter.setStyleSheet("color: #000020;")
            self.ui.qfProbeResultXlength.setText(
                "{:+0.3f}".format(
                    float(self.ui.qfProbeResultXmax.text())
                    - float(self.ui.qfProbeResultXmin.text())
                )
        ...
        if self.__yMin and self.__yMax:
            self.ui.qfProbeResultYcenter.setText(
                "{:+0.3f}".format(
                    (
                        float(self.ui.qfProbeResultYmax.text())
                        + float(self.ui.qfProbeResultYmin.text())
                    )
                    / 2
                )
            )
            self.ui.qfProbeResultYcenter.setStyleSheet("color: #000020;")
            self.ui.qfProbeResultYlength.setText(
                "{:+0.3f}".format(
                    float(self.ui.qfProbeResultYmax.text())
                    - float(self.ui.qfProbeResultYmin.text())
                )
    ...
    def on_sig_config_changed(self, data: str):
        self.log(
            ...
            self.tr("Grbl configuration updated: {}").format(data),
        )
    ...
    def on_arretUrgence(self):
        if self.__arretUrgence:
            ...
            self.__arretUrgence = True
            self.log(
                logSeverity.warning.value, self.tr("Emergency stop pressed: STOP !!!")
            )
    ...
    def action_btnConnect(self):
        if not self.__connectionStatus:
            ...
            self.ui.btnConnect.setText(
                self.tr("Connect")
            )  # La prochaine action du bouton sera pour connecter
    ...
    def on_sig_connect(self):
        ...
        if self.__connectionStatus:
            ...
            self.ui.lblConnectStatus.setText(
                self.tr("Connected to {}").format(
                    self.ui.cmbPort.currentText().split("-")[0].strip()
                )
            )
            self.ui.btnConnect.setText(
                self.tr("Disconnect")
            )  # La prochaine action du bouton sera pour deconnecter
            self.setEnableDisableConnectControls()
            # Active les groupes de controles de pilotage de Grbl
            self.setEnableDisableGroupes()
            self.ui.lblSerialLock.setStyleSheet(
                ".QLabel{border-radius: 3px; background: green;}"
            )
            self.ui.lblSerialActivity.setStyleSheet(
                ".QLabel{border-radius: 3px; background: green;}"
            )
            # Beep
            ...
            self.setEnableDisableConnectControls()
            self.ui.lblSerialLock.setStyleSheet(
                ".QLabel{border-radius: 3px; background: #35322f;}"
            )
            self.ui.lblSerialActivity.setStyleSheet(
                ".QLabel{border-radius: 3px; background: #35322f;}"
            )
    ...
    def on_feedOverride(self, value: int):
        adjustFeedOverride(
            int(self.ui.lblAvancePourcent.text()[:-1]), value, self.__grblCom
        )
        self.ui.lblAvancePourcent.setText("{}%".format(value))
        if self.ui.btnLinkOverride.isChecked() and (
            value != self.ui.dialBroche.value()
        ):
            self.ui.dialBroche.setValue(value)

    @pyqtSlot(int)
    def on_spindleOverride(self, value: int):
        adjustSpindleOverride(
            int(self.ui.lblBrochePourcent.text()[:-1]), value, self.__grblCom
        )
        self.ui.lblBrochePourcent.setText("{}%".format(value))
        if self.ui.btnLinkOverride.isChecked() and (
            value != self.ui.dialAvance.value()
        ):
    ...
    def on_btnLinkOverride(self):
        if self.ui.btnLinkOverride.isChecked() and (
            self.ui.dialAvance.value() != self.ui.dialBroche.value()
        ):
    ...
    def on_jog(self, cnButton, e):
        ...
        if jogDistance != 0:
            ...
            axis = cnButton.name()[
                -1
            ]  # L'axe est definit par le dernier caractere du nom du Bouton
            maxTravel = 0
    ...
    def on_btnSpinM3(self):
        ...
        self.ui.btnSpinM4.setEnabled(
            False
        )  # Interdit un changement de sens de rotation direct

    @pyqtSlot()
    def on_btnSpinM4(self):
        self.__grblCom.gcodeInsert("M4")
        self.ui.btnSpinM3.setEnabled(
            False
        )  # Interdit un changement de sens de rotation direct
    ...
    def on_btnFloodM7(self):
        if (
            self.__decode.get_etatArrosage() != "M7"
            and self.__decode.get_etatArrosage() != "M78"
        ):
    ...
    def on_btnFloodM8(self):
        if (
            self.__decode.get_etatArrosage() != "M8"
            and self.__decode.get_etatArrosage() != "M78"
        ):
            # Envoi "Real Time Command"
    ...
    def on_btnFloodM9(self):
        if (
            self.__decode.get_etatArrosage() == "M7"
            or self.__decode.get_etatArrosage() == "M78"
        ):
            # Envoi "Real Time Command"
            self.__grblCom.realTimePush(REAL_TIME_TOGGLE_MIST_COOLANT)
        if (
    ...
    def txtGCode_on_Change(self):
        if self.ui.txtGCode.text() == REAL_TIME_REPORT_QUERY:
            ...
            )  # Envoi direct ? sans attendre le retour chariot.
            self.ui.txtGCode.setSelection(0, len(self.ui.txtGCode.text()))
    ...
    def on_keyPressed(self, e):
        ...
        if QKeySequence(key + int(e.modifiers())) == QKeySequence("Ctrl+C"):
            ...
            if len(self.__gcodes_stack) > 0:
                ...
                self.__gcodes_stack_pos += 1
                if self.__gcodes_stack_pos >= 0 and self.__gcodes_stack_pos < len(
                    self.__gcodes_stack
                ):
                    self.__gcode_recall_flag = True
                    self.ui.txtGCode.setText(
                        self.__gcodes_stack[self.__gcodes_stack_pos]
                    )
                    self.ui.txtGCode.setSelection(0, len(self.ui.txtGCode.text()))
    ...
    def on_sig_log(self, severity: int, data: str):
        if severity == logSeverity.info.value:
            ...
            self.logCn5X.append(
                time.strftime("%Y-%m-%d %H:%M:%S") + " : Error   : " + data
            )
            if not self.ui.btnDebug.isChecked():
                self.ui.qtabConsole.setCurrentIndex(CN5X_TAB_LOG)
    ...
    def on_sig_status(self, data: str):
        ...
        # Masque de la boite de progression
        if (
            (self.__decode.get_etatMachine() == GRBL_STATUS_IDLE)
            and self.__pBox.isVisible()
            and self.__pBoxArmee
        ):
    ...
    def on_mnuA_propos(self):
        ...
        dlgApropos = cn5XAPropos(
            self.tr("Version {}.{}").format(APP_VERSION_STRING, APP_VERSION_DATE),
            self.__licenceFile,
        )
    ...
    def on_btnDebug(self):
        ...
        if self.ui.btnDebug.isChecked():
            ...
            self.ui.btnPausePooling.setEnabled(True)
            self.on_sig_debug(
                "cn5X++ (v{}.{}) : Starting debug.".format(
                    APP_VERSION_STRING, APP_VERSION_DATE
                )
            )
        else:
            self.on_sig_debug(
                "cn5X++ (v{}.{}) : Stop debugging.".format(
                    APP_VERSION_STRING, APP_VERSION_DATE
                )
            )
            if self.ui.mnuDebug_mode.isChecked():
    ...
    def on_btnPausePooling(self):
        self.log(
            logSeverity.info.value,
            "on_btnPausePooling({})".format(self.ui.btnPausePooling.isChecked()),
        )
    ...
    def startCycle(self, startFrom: int = 0):
        if self.ui.gcodeTable.model().rowCount() <= 0:
            self.log(
                logSeverity.warning.value, self.tr("Attempt to start an empty cycle...")
            )
        else:
    ...
    def pauseCycle(self):
        if self.ui.lblEtat.text() == GRBL_STATUS_HOLD1:
            self.log(
                ...
                self.tr("Holding in progress, can't restart now."),
            )
    ...
    def stopCycle(self):
        if self.ui.lblEtat.text() == GRBL_STATUS_HOLD0:
            ...
            self.log(
                logSeverity.info.value, self.tr("Holding cycle before stopping...")
            )
    ...
    def on_gcodeTableContextMenu(self, event):
        if self.__gcodeFile.isFileLoaded():
            ...
            startFromHereAction = QtWidgets.QAction(
                self.tr("Run from this line\t(F8)"), self
            )
            startFromHereAction.setShortcut("F8")
            startFromHereAction.triggered.connect(
                lambda: self.startFromGCodeSlotIndex(event)
            )
    ...
    def on_lblPosContextMenu(self, axis: str):
        self.cMenu = QtWidgets.QMenu(self)
        resetX = QtWidgets.QAction(
            self.tr("Place the {} origin of axis {} here").format(
                self.__decode.getG5actif(), self.__axisNames[axis]
            ),
            self,
        )
        resetX.triggered.connect(
            lambda: self.__grblCom.gcodePush(
                "G10P0L20{}0".format(self.__axisNames[axis])
            )
        )
        self.cMenu.addAction(resetX)
        resetAll = QtWidgets.QAction(
            self.tr("Place the {} origin of all axis here").format(
                self.__decode.getG5actif()
            ),
            self,
        )
        axesTraites = []
        ...
        self.cMenu.addSeparator()
        resetX = QtWidgets.QAction(
            self.tr("Jog axis {} to {} origin").format(
                self.__axisNames[axis], self.__decode.getG5actif()
            ),
            self,
        )
        cmdJog1 = CMD_GRBL_JOG + "G90G21F{}{}0".format(
            self.ui.dsbJogSpeed.value(), self.__axisNames[axis]
        )
        resetX.triggered.connect(lambda: self.__grblCom.gcodePush(cmdJog1))
        self.cMenu.addAction(resetX)
        resetAll = QtWidgets.QAction(
            self.tr("Jog all axis to {} origin").format(self.__decode.getG5actif()),
            self,
        )
        axesTraites = []
    ...
    def on_lblGXXContextMenu(self, piece: int):
        self.cMenu = QtWidgets.QMenu(self)
        setOrigineAll = QtWidgets.QAction(
            self.tr("Place the workpiece origin {} (G{})").format(
                str(piece), str(piece + 53)
            ),
            self,
        )
    ...
    def on_lblCoordContextMenu(self):
        self.cMenu = QtWidgets.QMenu(self)
        unitePouces = QtWidgets.QAction(
            self.tr("G90 - Absolute coordinates movements"), self
        )
        unitePouces.triggered.connect(lambda: self.__grblCom.gcodePush("G90"))
        self.cMenu.addAction(unitePouces)
        uniteMM = QtWidgets.QAction(
            self.tr("G91 - relative coordinates movements"), self
        )
    ...
    def createLangMenu(self):
        ...
        for translation in translations:
            self.langues.append(
                translation.getElementsByTagName("locale")[0].childNodes[0].nodeValue
            )
            label = translation.getElementsByTagName("label")[0].childNodes[0].nodeValue
            qm_file = (
                translation.getElementsByTagName("qm_file")[0].childNodes[0].nodeValue
            )
            flag_file = (
                translation.getElementsByTagName("flag_file")[0].childNodes[0].nodeValue
            )

            self.ui.actionLang.append(QtWidgets.QAction(self))
            self.ui.iconLang.append(QtGui.QIcon())
            self.ui.iconLang[l].addPixmap(
                QtGui.QPixmap(flag_file), QtGui.QIcon.Normal, QtGui.QIcon.Off
            )
    ...
    def on_sig_activity(self, val: bool):
        if val:
            ...
            self.ui.lblSerialActivity.setStyleSheet(
                ".QLabel{border-radius: 3px; background: red;}"
            )
        else:
    ...
    def setTranslator(self, langue: QLocale):
        ...
        if not translator.load(langue, "{}/i18n/cn5X".format(app_path), "."):
            self.log(
                logSeverity.error.value,
                self.tr("Locale ({}) not usable, using default to english").format(
                    langue.name()
                ),
            )
...
if __name__ == "__main__":
    ...
    print(
        "{} v{}.{} running from: {}".format(
            APP_NAME, APP_VERSION_STRING, APP_VERSION_DATE, app_path
        )
    )
