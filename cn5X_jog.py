...
class dlgJog(QObject):
    ...
    def __init__(self, grbl: grblCom, decoder, axisNumber: int, axisNames: list):
        ...
        self.activeColor = (
            self.di.dsbJogX.palette().color(QPalette.Active, QPalette.WindowText).name()
        )
        self.disabledColor = (
            self.di.dsbJogX.palette()
        ...
        self.di.btnJogX.changeIcon(
            ":/cn5X/images/btnJog{}.svg".format(self.__axisNames[0])
        )
        self.di.chkJogY.setText(self.tr("Jog {} axis".format(self.__axisNames[1])))
        self.di.btnJogY.changeIcon(
            ":/cn5X/images/btnJog{}.svg".format(self.__axisNames[1])
        )
        self.di.chkJogZ.setText(self.tr("Jog {} axis".format(self.__axisNames[2])))
        self.di.btnJogZ.changeIcon(
            ":/cn5X/images/btnJog{}.svg".format(self.__axisNames[2])
        )
        if self.__nbAxis > 3:
            self.di.chkJogA.setText(self.tr("Jog {} axis".format(self.__axisNames[3])))
            self.di.btnJogA.changeIcon(
                ":/cn5X/images/btnJog{}.svg".format(self.__axisNames[3])
            )
        ...
        if self.__nbAxis > 4:
            ...
            self.di.btnJogB.changeIcon(
                ":/cn5X/images/btnJog{}.svg".format(self.__axisNames[4])
            )
        ...
        if self.__nbAxis > 5:
            ...
            self.di.btnJogC.changeIcon(
                ":/cn5X/images/btnJog{}.svg".format(self.__axisNames[5])
            )
        ...
        self.di.chkJogX.stateChanged.connect(
            lambda: self.on_chkJog_changed(self.di.chkJogX, 0)
        )
        self.di.chkJogY.stateChanged.connect(
            lambda: self.on_chkJog_changed(self.di.chkJogY, 1)
        )
        self.di.chkJogZ.stateChanged.connect(
            lambda: self.on_chkJog_changed(self.di.chkJogZ, 2)
        )
        self.di.chkJogA.stateChanged.connect(
            lambda: self.on_chkJog_changed(self.di.chkJogA, 3)
        )
        self.di.chkJogB.stateChanged.connect(
            lambda: self.on_chkJog_changed(self.di.chkJogB, 4)
        )
        self.di.chkJogC.stateChanged.connect(
            lambda: self.on_chkJog_changed(self.di.chkJogC, 5)
        )

        self.di.dsbJogX.valueChanged.connect(
            lambda: self.on_dsbJog_changed(self.di.dsbJogX, self.di.chkJogX, 0)
        )
        self.di.dsbJogY.valueChanged.connect(
            lambda: self.on_dsbJog_changed(self.di.dsbJogY, self.di.chkJogY, 1)
        )
        self.di.dsbJogZ.valueChanged.connect(
            lambda: self.on_dsbJog_changed(self.di.dsbJogZ, self.di.chkJogZ, 2)
        )
        self.di.dsbJogA.valueChanged.connect(
            lambda: self.on_dsbJog_changed(self.di.dsbJogA, self.di.chkJogA, 3)
        )
        self.di.dsbJogB.valueChanged.connect(
            lambda: self.on_dsbJog_changed(self.di.dsbJogB, self.di.chkJogB, 4)
        )
        self.di.dsbJogC.valueChanged.connect(
            lambda: self.on_dsbJog_changed(self.di.dsbJogC, self.di.chkJogC, 5)
        )
    ...
    def on_btnJog(self, axisNum: int = None, spinBox: QDoubleSpinBox = None):
        ...
        if axisNum is not None:
            ...
            i = 0
            for chk, val in [
                (self.di.chkJogX, self.di.dsbJogX.value()),
    ...
    def jogCancel(self):
        ...
        self.__grblCom.realTimePush(
            REAL_TIME_JOG_CANCEL
        )  # Commande realtime Jog Cancel
