    ...
    def eventFilter(self, object, event):
        ...
        if event.type() == QtCore.QEvent.DynamicPropertyChange:
            ...
            if QtCore.QResource(pictureBaseName + ".svg").isValid():
                self.icon.addPixmap(
                    QtGui.QPixmap(pictureBaseName + ".svg"),
                    QtGui.QIcon.Normal,
                    QtGui.QIcon.Off,
                )
                if QtCore.QResource(pictureBaseName + "_down.svg").isValid():
                    self.iconDown.addPixmap(
                        QtGui.QPixmap(pictureBaseName + "_down.svg"),
                        QtGui.QIcon.Normal,
                        QtGui.QIcon.Off,
                    )
                else:
                ...
                if QtCore.QResource(pictureBaseName + "_light.svg").isValid():
                    self.iconLight.addPixmap(
                        QtGui.QPixmap(pictureBaseName + "_light.svg"),
                        QtGui.QIcon.Normal,
                        QtGui.QIcon.Off,
                    )
                else:
        ...
        if event.type() == QtCore.QEvent.Resize:
            self.setIconSize(
                QtCore.QSize(self.size().width() - 2, self.size().height() - 2)
            )
