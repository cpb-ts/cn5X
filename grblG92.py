...
class dlgG92(QObject):
    ...
    def __init__(self, grbl: grblCom, decoder, axisNumber: int, axisNames: list):
        ...
        self.di.chkAutoclose.setChecked(
            self.__settings.value("G92/autoCloseDialog", False, type=bool)
        )

        # Mémorise les couleurs standard des spin boxes
        self.activeColor = (
            self.di.dsbG92valeurX.palette()
        ...
        self.di.buttonBox.button(QDialogButtonBox.Close).clicked.connect(
            self.__dlgG92.close
        )
        ...
        self.di.chkDefineX.stateChanged.connect(
            lambda: self.on_chkDefine_changed(self.di.chkDefineX, 0)
        )
        self.di.chkDefineY.stateChanged.connect(
            lambda: self.on_chkDefine_changed(self.di.chkDefineY, 1)
        )
        self.di.chkDefineZ.stateChanged.connect(
            lambda: self.on_chkDefine_changed(self.di.chkDefineZ, 2)
        )
        self.di.chkDefineA.stateChanged.connect(
            lambda: self.on_chkDefine_changed(self.di.chkDefineA, 3)
        )
        self.di.chkDefineB.stateChanged.connect(
            lambda: self.on_chkDefine_changed(self.di.chkDefineB, 4)
        )
        self.di.chkDefineC.stateChanged.connect(
            lambda: self.on_chkDefine_changed(self.di.chkDefineC, 5)
        )

        self.di.dsbG92valeurX.valueChanged.connect(
            lambda: self.on_dsbG92valeur_changed(
                self.di.dsbG92valeurX, self.di.chkDefineX, 0
            )
        )
        self.di.dsbG92valeurY.valueChanged.connect(
            lambda: self.on_dsbG92valeur_changed(
                self.di.dsbG92valeurY, self.di.chkDefineY, 1
            )
        )
        self.di.dsbG92valeurZ.valueChanged.connect(
            lambda: self.on_dsbG92valeur_changed(
                self.di.dsbG92valeurZ, self.di.chkDefineZ, 2
            )
        )
        self.di.dsbG92valeurA.valueChanged.connect(
            lambda: self.on_dsbG92valeur_changed(
                self.di.dsbG92valeurA, self.di.chkDefineA, 3
            )
        )
        self.di.dsbG92valeurB.valueChanged.connect(
            lambda: self.on_dsbG92valeur_changed(
                self.di.dsbG92valeurB, self.di.chkDefineB, 4
            )
        )
        self.di.dsbG92valeurC.valueChanged.connect(
            lambda: self.on_dsbG92valeur_changed(
                self.di.dsbG92valeurC, self.di.chkDefineC, 5
            )
        )
    ...
    def on_dsbG92valeur_changed(self, dbsChanged, chkDefine, axisNumChanged):
        ...
        i = 0
        for dbs in [
            self.di.dsbG92valeurX,
            self.di.dsbG92valeurY,
            self.di.dsbG92valeurZ,
            self.di.dsbG92valeurA,
            self.di.dsbG92valeurB,
            self.di.dsbG92valeurC,
        ]:
            if dbs != dbsChanged:
                if i >= self.__nbAxis:
                    break
    ...
    def showDialog(self):
        ...
        self.__dlgG92.setFixedSize(
            self.__dlgG92.geometry().width(), self.__dlgG92.geometry().height()
        )
        self.__dlgG92.move(
    ...
    def on_chkAutoclose_toggled(self):
        self.__settings.setValue(
            "G92/autoCloseDialog", self.di.chkAutoclose.isChecked()
        )
    ...
    def on_btnSetOriginG92(self):
        ...
        if self.di.chkDefineX.isChecked():
            originGcode += "X{}".format(float(self.di.dsbG92valeurX.value()))
        if self.di.chkDefineY.isChecked():
            originGcode += "Y{}".format(float(self.di.dsbG92valeurY.value()))
        if self.di.chkDefineZ.isChecked():
            originGcode += "Z{}".format(float(self.di.dsbG92valeurZ.value()))
        if self.di.chkDefineA.isChecked():
            originGcode += "A{}".format(float(self.di.dsbG92valeurA.value()))
        if self.di.chkDefineB.isChecked():
            originGcode += "B{}".format(float(self.di.dsbG92valeurB.value()))
        if self.di.chkDefineC.isChecked():
            originGcode += "C{}".format(float(self.di.dsbG92valeurC.value()))
    ...
    def on_chkDefine_toogle(self, axis: str):
        ...
        if axis == "X":
            if self.di.chkDefineX.isChecked():
                self.di.dsbG92valeurX.setStyleSheet(
                    "color: {};".format(self.activeColor)
                )
            else:
                self.di.dsbG92valeurX.setStyleSheet(
                    "color: {};".format(self.disabledColor)
                )
        if axis == "Y":
            if self.di.chkDefineY.isChecked():
                self.di.dsbG92valeurY.setStyleSheet(
                    "color: {};".format(self.activeColor)
                )
            else:
                self.di.dsbG92valeurY.setStyleSheet(
        ...
        if axis == "Z":
            if self.di.chkDefineZ.isChecked():
                self.di.dsbG92valeurZ.setStyleSheet(
                    "color: {};".format(self.activeColor)
                )
            else:
                self.di.dsbG92valeurZ.setStyleSheet(
        ...
        if axis == "A":
            if self.di.chkDefineA.isChecked():
                self.di.dsbG92valeurA.setStyleSheet(
                    "color: {};".format(self.activeColor)
                )
            else:
                self.di.dsbG92valeurA.setStyleSheet(
        ...
        if axis == "B":
            if self.di.chkDefineB.isChecked():
                self.di.dsbG92valeurB.setStyleSheet(
                    "color: {};".format(self.activeColor)
                )
            else:
                self.di.dsbG92valeurB.setStyleSheet(
        ...
        if axis == "C":
            if self.di.chkDefineC.isChecked():
                self.di.dsbG92valeurC.setStyleSheet(
                    "color: {};".format(self.activeColor)
                )
            else:
                self.di.dsbG92valeurC.setStyleSheet(
