...
from PyQt5.QtCore import QCoreApplication, QObject, QSettings, Qt, pyqtSignal, pyqtSlot
...
class qwProgressBox(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.pBoxLblStart.setText(
            self.tr("GCode started at: {}").format(debut.strftime("%A %x %H:%M:%S"))
        )
        self.pBoxLblStart.setStyleSheet("color: white;")
        self.pBoxLblStart.adjustSize()
        self.pBoxBtnHeader.setGeometry(
            2, 2, self.pBoxLblStart.width() + 36, self.pBoxBtnHeader.height()
        )
        self.pBoxLblStart.setGeometry(
            20,
            int((self.pBoxBtnHeader.height() - self.pBoxLblStart.height()) / 2),
            self.pBoxLblStart.width(),
            self.pBoxLblStart.height(),
        )
        ...
        self.pBoxProgress.setGeometry(
            20,
            self.pBoxBtnHeader.geometry().y() + self.pBoxBtnHeader.height() + 9,
            self.pBoxLblStart.width(),
            20,
        )
        ...
        self.pBoxLblElapse.setGeometry(
            20,
            self.pBoxLblComment.geometry().y() + self.pBoxLblComment.height() + 3,
            self.pBoxLblStart.width(),
            20,
        )
        ...
        pBoxHauteur = (
            self.pBoxLblElapse.geometry().y() + self.pBoxLblElapse.height() + 6
        )
...
class elapseThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self._stopevent.isSet():
            ...
            hours, remainder = divmod(total_seconds, 60 * 60)
            minutes, seconds = divmod(remainder, 60)
            self.pBoxLblElapse.setText(
                "{} {:02d}:{:02d}:{:02d}".format(
                    self.initialText, hours, minutes, seconds
                )
            )
...
class btnHeader(QtWidgets.QPushButton):
    ...
    def mouseMoveEvent(self, e):
        ...
        if newX < 0:
            newX = 0
        if newX > self.__parent.parent().width() - self.__parent.width():
            newX = self.__parent.parent().width() - self.__parent.width()
        newY = self.__parent.pos().y() + (e.y() - self.y0)
        if newY < 0:
            newY = 0
        if newY > self.__parent.parent().height() - self.__parent.height():
            newY = self.__parent.parent().height() - self.__parent.height()
