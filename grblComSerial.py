...
class grblComSerial(QObject):
    ...
    sig_connect = pyqtSignal(
        bool
    )  # Message emis a la connexion (valeur = True) et a la deconnexion ou en cas d'erreur de connexion (valeur = False)
    sig_log = pyqtSignal(
        int, str
    )  # Message de fonctionnement du composant grblComSerial, renvoie : logSeverity, message string
    sig_init = pyqtSignal(
        str
    )  # Emis a la reception de la chaine d'initialisation de Grbl, renvoie la chaine complete
    sig_ok = pyqtSignal()  # Emis a la reception de la chaine "ok"
    sig_error = pyqtSignal(
        int
    )  # Emis a la reception d'une erreur Grbl, renvoie le N° d'erreur
    sig_alarm = pyqtSignal(
        int
    )  # Emis a la reception d'une alarme Grbl, renvoie le N° d'alarme
    sig_status = pyqtSignal(
        str
    )  # Emis a la reception d'un message de status ("<...|.>"), renvoie la ligne complete
    sig_config = pyqtSignal(str)  # Emis a la reception d'une valeur de config ($XXX)
    sig_data = pyqtSignal(
        str
    )  # Emis a la reception des autres donnees de Grbl, renvoie la ligne complete
    ...
    sig_activity = pyqtSignal(
        bool
    )  # Emis lors de l'émission/réception de données sur le port série
    sig_serialLock = pyqtSignal(
        bool
    )  # Emis a chaque changement de self.__okToSendGCode
    ...
    def abort(self):
        ...
        self.sig_log.emit(
            logSeverity.info.value, self.tr("grblComSerial.py: abort received.")
        )
        self.__abort = True
    ...
    def __sendData(self, buff: str):
        ...
        timeout = 10 + (
            2 * tempNecessaire
        )  # 2 fois le temps necessaire + 10 millisecondes
        self.__comPort.write_timeout = timeout
        # Ecriture sur le port serie
        self.sig_debug.emit(
            "grblComSerial.__sendData(), T = {} : timeout = {}".format(
                time.time() * 1000, timeout
            )
        )
        ...
        try:
            ...
            self.sig_log.emit(
                ...
                self.tr(
                    "grblComSerial: Error when sending data: timeout, err# = {}"
                ).format(self.__comPort.error()),
            )
        except:
            self.sig_log.emit(
                logSeverity.error.value, self.tr("grblComSerial: Unknown error")
            )
        else:
            self.sig_debug.emit(
                self.tr("grblComSerial: Data sent, T = {}".format(time.time() * 1000))
            )
    ...
    def __traileLaLigne(self, l, flag=COM_FLAG_NO_FLAG):
        ...
        if (
            l[:5] == "Grbl " and l[-5:] == "help]"
        ):  # Init string : Grbl 1.1f ['$' for help]
            ...
            l[:1] == "$" or l[:5] == "[VER:" or l[:5] == "[AXS:" or l[:5] == "[OPT:"
        ):  # Setting output
    ...
    def __openComPort(self):
        ...
        openResetTime = (
            2000  # Time for sending soft reset if init string is not receive from Grbl
        )
        ...
        try:
            ...
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblComSerial.__openComPort(): Error opening serial port : {0}"
                ).format(err),
            )
            self.sig_debug.emit(
                self.tr(
            ...
            ValueError
        ) as err:  # – Will be raised when parameter are out of range e.g. baud rate, data bits.
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr(
                    "grblComSerial.__openComPort(): Parameter out of range : {0}"
                ).format(err),
            )
            self.sig_debug.emit(
                self.tr(
            ...
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr("grblComSerial.__openComPort(): Unexpected error : {}").format(
                    sys.exc_info()[0]
                ),
            )
            self.sig_debug.emit(
        ...
        self.sig_log.emit(
            ...
            self.tr("grblComSerial.__openComPort(): comPort {} open.").format(
                self.__comPort.port
            ),
        )
        self.sig_debug.emit(
        ...
        self.sig_debug.emit(
            self.tr(
                "grblComSerial.__openComPort(): Wait for Grbl init... T = {:0.0f} ms..."
            ).format(tDebut)
        )
        ...
        while self.__comPort.in_waiting == 0:
            ...
            if now > tDebut + openReceiveTimeout:
                self.sig_log.emit(
                    logSeverity.error.value,
                    self.tr(
                        "grblComSerial.__openComPort(): timeout! No reply from Grbl."
                    ),
                )
                self.sig_debug.emit(
                    self.tr(
        ...
        while True:
            while self.__comPort.in_waiting:
                try:
                    ...
                    self.sig_log.emit(
                        logSeverity.error.value,
                        self.tr(
                            "grblComSerial.__openComPort(): Read error: {}".format(err)
                        ),
                    )
                    self.sig_debug.emit(
                        self.tr(
                ...
                try:
                    l = buff.decode().strip()
                    self.sig_debug.emit(
                        self.tr('grblComSerial.__openComPort(): line received: "')
                        + l
                        + '"'
                    )
                    if (
                        l[:5] == "Grbl " and l[-5:] == "help]"
                    ):  # Init string : Grbl V.Mx ['$' for help]
                        self.sig_debug.emit(
                            self.tr(
                                "grblComSerial.__openComPort(): Grbl init string received in {:0.0f} ms, OK."
                            ).format(time.time() * 1000 - tDebut)
                        )
                        self.sig_init.emit(l)
                        self.__initOK = True
                    else:
                    ...
                    self.sig_log.emit(
                        ...
                        self.tr(
                            "grblComSerial.__openComPort(): utf-8 decode error, buff={}".format(
                                buff
                            )
                        ),
                    )
            if not self.__initOK:
                ...
                if now > tDebut + (openResetTime) and not tReset:
                    ...
                    self.sig_debug.emit(
                        self.tr(
                            "grblComSerial.__openComPort(): No response from Grbl after {:0.0f}ms, sending soft reset..."
                        ).format(openResetTime)
                    )
                    self.__sendData(REAL_TIME_SOFT_RESET)
                    self.__sendData("\r\n")
                    tReset = True
                if now > tDebut + openMaxTime:
                    self.sig_log.emit(
                        logSeverity.error.value,
                        self.tr(
                            "grblComSerial.__openComPort(): Grbl initialization: Timeout!"
                        ),
                    )
                    self.sig_debug.emit(
                        self.tr(
                            "grblComSerial.__openComPort(): openMaxTime ({}ms) timeout elapsed !"
                        ).format(openMaxTime)
                    )
                    self.sig_log.emit(
                        logSeverity.error.value,
                        self.tr(
                            "grblComSerial.__openComPort(): Grbl's init string not received or unknown Grbl version."
                        ),
                    )
    ...
    def __mainLoop(self):
        ...
        while True:
            ...
            while self.__comPort.in_waiting:
                ...
                try:
                    ...
                    if l.find("ok") >= 0:
                        self.sig_debug.emit(
                            self.tr("grblComSerial: __mainLoop(): ok received")
                        )
                    if l.find("error") >= 0:
                        self.sig_debug.emit(
                            self.tr(
                                "grblComSerial: __mainLoop(): error Grbl received [{}]."
                            ).format(l)
                        )
                    if l.find("ALARM") >= 0:
                        self.sig_debug.emit(
                            self.tr(
                                "grblComSerial: __mainLoop(): ALARM Grbl received [{}]."
                            ).format(l)
                        )
                    if l != "":
                    ...
                    self.sig_log.emit(
                        logSeverity.error.value,
                        self.tr(
                            "grblComSerial.__mainLoop: Timeout when reading serial port!"
                        ),
                    )
                    ...
                    self.sig_log.emit(
                        logSeverity.error.value,
                        self.tr(
                            "grblComSerial.__mainLoop(): Unexpected exception when reading serial port!"
                        ),
                    )
                except UnicodeDecodeError:
                    # Trace l'erreur et ignore...
                    self.sig_log.emit(
                        logSeverity.warning.value,
                        self.tr(
                            "grblComSerial.__mainLoop(): utf-8 decode error, buff={}".format(
                                buff
                            )
                        ),
                    )
            ...
            if self.__abort:
                self.sig_log.emit(
                    logSeverity.info.value,
                    self.tr(
                        "grblComSerial.__mainLoop(): Abort received, closing the thread..."
                    ),
                )
            ...
            if self.__pooling:
                if (
                    time.time() - self.__lastQueryTime
                ) * 1000 > GRBL_QUERY_DELAY and self.__initOK:
                    if len(self.__querySequence[self.__queryCounter]) == 1:
                        ...
                        if self.__grblStatus == GRBL_STATUS_IDLE:
                            self.gcodeInsert(
                                self.__querySequence[self.__queryCounter],
                                COM_FLAG_NO_OK | COM_FLAG_NO_ERROR,
                            )
                    self.__lastQueryTime = time.time()
        ...
        self.sig_log.emit(
            logSeverity.info.value,
            self.tr("grblComSerial.__mainLoop(): Closing serial port."),
        )
        self.sig_connect.emit(False)
        ...
        self.sig_log.emit(
            logSeverity.info.value, self.tr("grblComSerial.__mainLoop(): End.")
        )
    ...
    def run(self):
        ...
        self.sig_log.emit(
            logSeverity.info.value,
            self.tr('grblComSerial.run(): Running "{}" from thread #{}.').format(
                thread_name, hex(thread_id)
            ),
        )
        ...
        if self.__openComPort():
            ...
            self.sig_log.emit(
                logSeverity.error.value,
                self.tr("grblComSerial.run(): Unable to open serial port!"),
            )
            # Emission du signal de fin
            self.sig_log.emit(
                logSeverity.info.value, self.tr("grblComSerial.run(): End.")
            )
